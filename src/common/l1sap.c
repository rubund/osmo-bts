...
static int to_gsmtap(struct gsm_bts_trx *trx, struct osmo_phsap_prim *l1sap)
{
	...
	uint8_t *data;
	int len;
	uint8_t chan_type = 0, fn = 0, tn = 0, ss = 0;
...
#warning Set BS_AG_BLKS_RES
	...
	if ((chan_type & GSMTAP_CHANNEL_ACCH) && !gsmtap_sapi_acch)
		return 0;
	else if (!((1 << (chan_type & 31)) & gsmtap_sapi_mask))
		return 0;
	 ...
	 struct osmo_phsap_prim *l1sap, struct ph_data_param *rts_ind)
{
	...
	if (L1SAP_IS_CHAN_BCCH(chan_nr)) {
		...
		if (rc < 0) {
			...
			/* The +2 is empty space where the DSP inserts the L1 hdr */
			if (L1SAP_IS_LINK_SACCH(link_id))
				memcpy(p + 2, pp.oph.msg->data, GSM_MACBLOCK_LEN - 2);
			else {
