...
static int handle_mph_time_ind(struct femtol1_hdl *fl1, struct msgb *msg,
...
static int handle_ph_readytosend_ind(struct femtol1_hdl *fl1, struct msgb *msg,
				     GsmL1_PhReadyToSendInd_t *rts_ind)
{
	...
	if (chan_nr) {
		...
		if (rts_ind->sapi == GsmL1_Sapi_Sacch)
			...
			link_id = 0;
		rc = msgb_trim(msg, sizeof(*l1sap));
		if (rc < 0)
			MSGB_ABORT(msg, "No room for primitive\n");
...
static int handle_ph_data_ind(struct femtol1_hdl *fl1, struct msgb *msg,
				GsmL1_PhDataInd_t *data_ind)
{
	...
	if (data_ind->sapi == GsmL1_Sapi_TchF
	 || data_ind->sapi == GsmL1_Sapi_TchH) {
		...
		return l1if_tch_rx(trx, chan_nr, msg);
	}
	...
	msgb_pull(msg, data - msg->data);
	/* trim remaining data to it's size, to get rid of upper header part */
	rc = msgb_trim(msg, len);
	if (rc < 0)
		MSGB_ABORT(msg, "No room for primitive data\n");
	msg->l2h = msg->data;
	/* push new l1 header */
	msg->l1h = msgb_push(msg, sizeof(*l1sap));
	/* fill header */
	l1sap = msgb_l1sap_prim(msg);
...
static int handle_ph_ra_ind(struct femtol1_hdl *fl1, struct msgb *msg,
...
static int l1if_handle_ind(struct femtol1_hdl *fl1, struct msgb *msg)
{
	...
	case GsmL1_PrimId_MphTimeInd:
		return handle_mph_time_ind(fl1, msg, &l1p->u.mphTimeInd);
		break;
	...
	case GsmL1_PrimId_PhReadyToSendInd:
		return handle_ph_readytosend_ind(fl1, msg,
			&l1p->u.phReadyToSendInd);
		break;
	case GsmL1_PrimId_PhDataInd:
		return handle_ph_data_ind(fl1, msg, &l1p->u.phDataInd);
		break;
	case GsmL1_PrimId_PhRaInd:
		return handle_ph_ra_ind(fl1, msg, &l1p->u.phRaInd);
		break;
	default:
		break;
	}
